...
CUR_CLUSTER_PG=$(aws rds describe-db-clusters \
  ...
  --query "DBClusters[0].DBClusterParameterGroup" -r text)

# discover one member's instance parameter group as template
CUR_INSTANCE_PG=$(aws rds describe-db-instances \
  --query "DBInstances[?DBClusterIdentifier=='$CLUSTER_ID']|[0].DBParameterGroups[0].DBParameterGroupName" -r text)
...
if ! aws rds describe-db-cluster-parameter-groups \
      --query "DBClusterParameterGroups[?DBClusterParameterGroupName=='$NEW_CLUSTER_PG']" -o json | jq -e 'length>0' >/dev/null; then
  ...
  FAMILY=$(aws rds describe-db-cluster-parameter-groups \
    ...
    --query "DBClusterParameterGroups[0].DBParameterGroupFamily" -r)
...
if ! aws rds describe-db-parameter-groups \
      --query "DBParameterGroups[?DBParameterGroupName=='$NEW_INSTANCE_PG']" -o json | jq -e 'length>0' >/dev/null; then
  FAMILY_I=$(aws rds describe-db-parameter-groups \
    --db-parameter-group-name "$CUR_INSTANCE_PG" \
    --query "DBParameterGroups[0].DBParameterGroupFamily" -r)
