...
export const handler = async () => {
  ...
  if (env.CDC_USER_SECRET) {
    ...
    if (!cdcRaw) {
      throw new Error {
        `Secret '${env.CDC_USER_SECRET}' has no SecretListEntryFilterSensitiveLog; can not configure CDC username.`
      }
    }
  }
  ...
  try {
    ...
    // GRANT SELECT ON ALL TABLES IN SCHEMA support_user TO support_bi_cdc;
    // Grant RDS replication capability (Aurora/RDS-PostgreSQL)
    await query(
      cdcDbConn,
      `GRANT rds_replication, rds_superuser TO ${cdcUsername}`,
    );
    await query(cdcDbConn, `CREATE PUBLICATION cdc_publication`);
  } finally {
    await cdcDbConn.end();
  }
