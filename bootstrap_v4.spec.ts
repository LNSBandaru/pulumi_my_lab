...
describe('Handler - Unit', () => {
  ...
  describe('handler', () => {
    it('should complete happy path', async () => {
      ...
      expect(adminCalls.length, 'expected 2 admin clients (initial + CDC reconnect)')
        .to.be.at.least(2);
      ...
      expect(appDbCalls.length, 'expected at least one pg.Client for app_database')
        .to.be.greaterThan(0);
      ...
      expect(serviceClientStub.connect.callCount, 'service + CDC same-DB connects')
        .to.be.at.least(2);
      ...
      expect(serviceClientStub.end.callCount, 'service + CDC same-DB ends')
        .to.be.at.least(2);
